...
if [ "$MVN_ARG_LINE" != "" ] ; then
    ...
    "$mvnBin" -v
    echo
    projects=( "*-model" "*-kjar" "document-business-service-service")
...
if [[ "$@" =~ "docker" ]]; then
    ...
    docker run -d -p 8090:8090 --name document-business-service-service apps/document-business-service-service:1.0-SNAPSHOT
elif [[ "$@" =~ "openshift" ]]; then
    echo "Launching the application on OpenShift..."
    
    oc new-app document-business-service-service:1.0-SNAPSHOT
    oc expose svc/document-business-service-service
else

	echo "Launching the application locally..."
	pattern="document-business-service-service"
	files=( $pattern )
	cd ${files[0]}
