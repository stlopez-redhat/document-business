...
if "%*" == "" (
	...
	for %%s in ("-model" "-kjar" "document-business-service-service") do (
...
:startapp
	if not x%mavenInput:docker=%==x%mavenInput% (
		...
		call docker run -d -p 8090:8090 --name document-business-service-service apps/document-business-service-service:1.0-SNAPSHOT
	) else if not x%mavenInput:openshift=%==x%mavenInput% (
		echo Launching the application on OpenShift...
		call oc new-app document-business-service-service:1.0-SNAPSHOT
		call oc expose svc/document-business-service-service
	) else (
		echo "Launching the application locally..."
		setlocal EnableDelayedExpansion
		cd document-business-service-service
		cd target
